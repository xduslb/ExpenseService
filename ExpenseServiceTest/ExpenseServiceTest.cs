...
namespace Expense.Service.Test
{
    public class ExpenseServiceTest
    {
        ...
        {
            // given
            var project = new Project(ProjectType.EXTERNAL, "Other Project");
            // when
            var result = ExpenseService.GetExpenseCodeByProjectTypeAndName(project);
            // then
            Assert.Equal(ExpenseType.OTHER_EXPENSE, result);
        }
        ...
        {
            // given
            var project = new Project(ProjectType.UNEXPECTED_PROJECT_TYPE, "Other Project");
            // when
            var result = new Action(() => ExpenseService.GetExpenseCodeByProjectTypeAndName(project));
            // then
            Assert.Throws<UnexpectedProjectTypeException>(result);
        }
    }
}